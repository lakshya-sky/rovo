...
// #[derive(Debug)]
// pub struct Linear {
//     pub ws: Option<NewTensor>,
//     pub bs: Option<NewTensor>,
...
//     fn reset(&mut self) {
//         let ws = NewTensor::empty(&[self.options.out_features, self.options.in_features]);
//         super::module::register_parameter(&ws, true);
//         self.ws = Some(ws);
//         if self.options.bias {
//             let bs_ = NewTensor::empty(&[self.options.out_features]);
...
//         let x = NewTensor::from_scalar(&[2, 4], 1.5, true);
